...
class LogsController extends Controller
{
    public function index()
    {
        ...
        $peak_hour      = "00:00";
        $data           = array();
        return view('report.students.user-logs', compact('data', 'inputName', 'inputLastName', 'fromInputDate', 'toInputDate', 'showTable', 'peak_hour'));
    }
    public function retrieve(Request $request)
    {
        ...
        if($pdfBtn == 'activated'){
            ...
            if(!count($data)) return redirect()->route('report.user')->with('toast-error', 'There are no data to generate.');
            $this->generatePDF($data);
            return redirect()->route('report.user')->with('toast-success', 'PDF successfully generated.');
        } else if($findBtn == 'activated'){
            if(strlen($fromInputDate) != 0 || strlen($inputName) != 0 || strlen($inputLastName) != 0){
                $data = $this->generateData($request);
            } else {
                return redirect()->route('report.user')->with('toast-warning', 'Please enter at least one search criteria.');
            }
            ...
            return redirect()->route('report.user')->with('toast-success', 'Excel successfully generated.');
        }
        if(!count($data)) return redirect()->route('report.user')->with('toast-error', 'No data found.');
        if(count($data) > 0) $time_in_arr = $data->pluck('log_time')->toArray();
        $peak_hour = $this->findPeakHour($time_in_arr) . ":00";
        return view('report.students.user-logs', compact('data', 'inputName', 'inputLastName', 'fromInputDate', 'toInputDate', 'peak_hour'));
    }
